> **この文書は凍結されています。**
> フェーズ1（画面遷移の骨組みを作った回）の実装指示書であり、履歴として残しています。
> 認証・ヘッダー・ダッシュボードの追加や、ディレクトリ構成の変更により
> **現状とは一致しません**（`src/` は存在しない、`actions/` は `presentation/` に、
> DBは認証で使用中、`user/` は作成済み、など）。
>
> **現行仕様は [`docs/spec.md`](docs/spec.md) を参照してください。**
